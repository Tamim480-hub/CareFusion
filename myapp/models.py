...
class User(AbstractUser):
    ...
    phone = models.CharField(max_length=15, blank=True, null=True)  # null=True যোগ করুন
    address = models.TextField(blank=True, null=True)  # null=True যোগ করুন
    profile_picture = models.ImageField(upload_to='profiles/', null=True, blank=True)
    date_of_birth = models.DateField(null=True, blank=True)
    blood_group = models.CharField(max_length=5, blank=True, null=True)  # null=True যোগ করুন
...
class Appointment(models.Model):
    ...
    def __str__(self):
        return f"{self.patient.get_full_name()} - {self.doctor.full_name}"
...
class ICUBooking(models.Model):
    patient = models.ForeignKey(User, on_delete=models.CASCADE, related_name='icu_bookings')
...
class EmergencyRequest(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.assigned_ambulance:
            self.assigned_ambulance = f'AMB-{EmergencyRequest.objects.count() + 1:04d}'
...
# myapp/models.py - সব মডেলের শেষে এই মডেলটি যোগ করুন

# myapp/models.py - UserProfile মডেলটি এইভাবে পরিবর্তন করুন
